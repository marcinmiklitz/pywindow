        ...
        with open(self.filepath) as trajectory_file:
            with closing(
                ...
                molsys = MolecularSystem.load_system(
                    decoded_frame, "_".join([self.system_id, str(frame_no)])
                )
        ...
        with open(self.filepath) as trajectory_file:
            ...
            with closing(
                ...
                while binary_step < len(file_binary_map):
                    ...
                    progress = round(
                        binary_step * 100 / len(file_binary_map), 0
                    )
                    string_line = (
                        binary_line.decode("utf-8").strip("\n").split()
                    )
                    ...
                    if line == 1:
                        if string_line[0] != "DLFIELD":
                            self.check_log = " ".join(
                                (self.check_log, f"Line {line}:", warning_1)
                            )
                    ...
                    if line == 2:
                        if len(string_line) != 3:
                            self.check_log = " ".join(
                                (self.check_log, f"Line {line}:", warning_2)
                            )
